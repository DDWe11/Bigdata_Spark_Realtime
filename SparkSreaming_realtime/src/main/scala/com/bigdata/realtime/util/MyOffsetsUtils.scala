...
/*
 * @Description:Offset管理工具类，用于往redis中存储和读取offset
 * @管理方案
 *  1.  后置提交偏移量 -> 手动控制偏移量提交
 *  2.  手动控制偏移量提交 -> SparkStreaming提供了手动提交方案（不能用）
 *  3.  手动的提取偏移量维护到redis中
 *          -> 从kafka中消费到数据，先提取偏移量
 *          -> 等数据成功写出后，将偏移量维护到redis中
 *          -> 从kafka中消费数据前，先到redis中读取偏移量，使用读取到的偏移量到kafka中消费数据
 *  4.  手动的将偏移量存储到redis中，每次消费数据需要使用存储的offset进行消费，每次消费数据后，
 *      要将本次消费的offset存储到redis中
 */
object MyOffsetsUtils {
    //Function1:往Redis存储offset
        //存的offset从哪来？从消费到的数据中提取出来的，传入到该方法中。offsetRanges: Array[OffsetRange]
        //offset的结构是什么？ Kafka中offset维护的结构；groupId + topic + partition => offset
                            //从传入进来的offset中提取关键信息
        //在redis中怎么存?   类型: hash    key : groupId + topic   value: partition - offset  ， partition - offset
                            //写入API: hset / hmset   读取API: hgetall  是否过期: 不过期
...
//            jedis.hset(redisKey,offsets)
    ...
    //Function2:从Redis读取offset
    //如何让SparkStreaming通过指定的offset进行消费? -> com.bigdata.realtime.util.MyKafkaUtils.getKafkaDStream
    //SparkStreaming要求的offset的格式:Map[TopicPartition ,Long]
